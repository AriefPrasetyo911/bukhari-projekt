                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-6">
                                <button class="btn btn-success btn-sm float-right" data-toggle="modal" data-target="#TambahSoalTKP">Tambah Paket TKP</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="TambahSoalTKPLabel">Tambah Paket Soal TKP</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form action="{{route('try-out.tambah-soal.tkp')}}" method="POST">
            @csrf
...
<div class="modal fade" id="EditSoalTWK" data-backdrop="static" tabindex="-1" aria-labelledby="EditSoalTWKLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        ...
        <form action="{{route('try-out.update-soal-tkp')}}" method="POST" enctype="multipart/form-data">
            @csrf
            {{ method_field('PATCH') }}
            <input type="hidden" name="tkp_id" id="tkp_id">
            <div class="modal-body">
                <div class="form-group">
    ...
    <script>
        $(document).ready(() => {
            var table_dashboard = $('.table-dashboard').DataTable({
                ...
                lengthChange: true,
                ajax: {
                    url: '{!! route('try-out.get-soal-tkp') !!}',
            ...
            $('.table-dashboard').on("click", '#editTKP', function(){
                $('#EditSoalTWK').modal('show');
                var idVal = $(this).data('id');
                console.log(idVal);
                $.get("/administrator/try-out/tkp/edit/"+idVal, (data) => {
                    $("#tkp_id").val(idVal);
            ...
            $('body').on('click', '.delete-TKP-btn', function (e){
                var tkp_id = $(this).data("id");
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: "/administrator/try-out/tkp/hapus-soal/"+tkp_id,
